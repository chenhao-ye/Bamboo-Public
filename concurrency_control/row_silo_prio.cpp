...
retry:
	...
	if (v.is_locked()) {
		...
		printf("[thd-%lu txn-%lu] row %p TID is locked. retry. \n[thd-%lu txn-%lu] TID: latch-%1u prio_ver-%1u, prio-%2u, ref_cnt-%1u, data_ver-%u\n", txn->get_thd_id(), txn->get_txn_id(), this->_row, txn->get_thd_id(), txn->get_txn_id(), v.is_locked(), v.get_prio_ver(), v.get_prio(), v.get_ref_cnt(), v.get_data_ver());
		if ((++retry_count) >= 500)
			exit(EXIT_FAILURE);
#endif
		PAUSE
