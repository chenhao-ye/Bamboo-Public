...
do
	python test.py experiments/binary_prio.json ZIPF_THETA=${zipf} THREAD_CNT=${thd} CC_ALG=${alg} DUMP_LATENCY=false > outputs/${i}_${alg}_YCSB_${zipf}_${thd}_output.txt
done
...
for alg in SILO SILO_PRIO
do
	python test.py experiments/binary_prio.json THREAD_CNT=${thd} CC_ALG=${alg} WORKLOAD=TPCC DUMP_LATENCY=false > outputs/${i}_${alg}_TPCC_${thd}_output.txt
done
